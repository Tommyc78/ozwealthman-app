...
export function SuperProjectionScreen() {
  ...
  const { data, superSettings, updateSuper } = useAppData();
  const base = getSuperProjection(data);
  const extra = getSuperProjection(data, 100, 'weekly');
  ...
  const [tpdInsurance, setTpdInsurance] = useState(String(superSettings.tpd_premium * 12));
  const [incomeProtection, setIncomeProtection] = useState(String(superSettings.ip_premium * 12));
  ...
  return (
    <Screen contentContainerStyle={styles.screen}>
      ...
      <Panel style={styles.formPanel}>
        ...
        <View style={styles.chipRow}>
          {(['weekly', 'monthly'] as const).map((item) => (
            <Pressable
              ...
              style={[
                styles.chip,
                { backgroundColor: frequency === item ? colors.accent : colors.surfaceRaised, borderColor: frequency === item ? colors.accent : colors.border },
              ]}
            >
        ...
        <MetricRow
          label="Net annual modelling impact"
          value={formatCurrency(configured.netAnnualImpact)}
          tone={configured.netAnnualImpact >= 0 ? 'positive' : 'danger'}
        />
            ...
            <View style={styles.actionRow}>
              <Pressable
                onPress={() => {
                  ...
                  updateSuper({
                    tpd_premium: (Number(tpdInsurance) || 0) / 12,
                    ip_premium: (Number(incomeProtection) || 0) / 12,
                    salary_sacrifice: Number(extraContribution) || superSettings.salary_sacrifice,
                  });
              ...
              <Pressable
                onPress={() => setPendingScenario(undefined)}
      ...
      <Panel>
        ...
        <Text subtle>
          The AI coach can explain scenarios, but the projection values come from the calculation service using stored balance,
          salary and contribution assumptions.
        </Text>
      </Panel>
    </Screen>
  );
