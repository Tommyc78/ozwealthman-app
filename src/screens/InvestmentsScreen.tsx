...
export function InvestmentsScreen() {
  ...
  const personalAccountIds = useMemo(
    () => new Set(data.accounts.filter((account) => !account.is_smsf).map((account) => account.id)),
    [data.accounts],
  );
  ...
  const personalBullion = data.bullionLots
  ...
  const propertyEquity = personalProperty.reduce(
    (total, property) => total + property.current_value - property.loan_balance,
    0,
  );
  ...
  const projectionChartPoints = projection.points.filter(
    (point, index) => index === 0 || point.age === projection.targetPoint.age || point.age === 67 || point.age % 5 === 0,
  );
  ...
  return (
    <Screen contentContainerStyle={styles.screen}>
      ...
      <Panel style={[styles.hero, { backgroundColor: colors.surfaceRaised, borderColor: colors.accent }]}>
        ...
        <Text subtle>
          Separate from super and SMSF. This area is for personal ETFs, crypto, bullion, cash, property and taxable
          investment records.
        </Text>
      ...
      <Panel style={styles.projectionPanel}>
        <View style={styles.projectionTop}>
          <View style={styles.projectionCopy}>
            ...
            <Text subtle>
              Model where the taxable portfolio could land before super access, including personal property, ETFs,
              crypto, metals, cash and regular new investing.
            </Text>
          </View>
        ...
        <View style={styles.projectionMetrics}>
          ...
          <InlineMetric
            ...
            value={formatCurrency(projection.projectedMonthlyIncome)}
            color={projection.monthlyIncomeGap >= 0 ? colors.success : colors.warning}
          />
        </View>
        ...
        <View style={styles.chartWrap}>
          <LineChart
            data={{
              ...
              datasets: [
                { data: projectionChartPoints.map((point) => Math.round(point.totalBalance)), color: () => colors.accentStrong, strokeWidth: 3 },
                { data: projectionChartPoints.map((point) => Math.round(point.shares + point.cash)), color: () => colors.chartTwo, strokeWidth: 2 },
              ],
        ...
        <View style={styles.chipRow}>
          {(['weekly', 'fortnightly', 'monthly', 'annual'] as ContributionFrequency[]).map((frequency) => (
            <Pressable
              ...
              style={[styles.chip, { backgroundColor: projectionInputs.investmentFrequency === frequency ? colors.accent : colors.surfaceRaised, borderColor: projectionInputs.investmentFrequency === frequency ? colors.accent : colors.border }]}
            >
      ...
      <Panel style={styles.formPanel}>
        <Text subtle>
          Use this as the front door for new taxable holdings. Later this panel will create confirmed Supabase records
          through the AI/tool flow.
        </Text>
        ...
        <View style={styles.inputGrid}>
          <TextInput placeholder="Symbol, asset or property" placeholderTextColor={colors.muted} value={entry.label} onChangeText={(value) => setEntry((current) => ({ ...current, label: value }))} style={[styles.textInput, { color: colors.text, borderColor: colors.border, backgroundColor: colors.surfaceRaised }]} />
          <TextInput placeholder="Units / quantity" placeholderTextColor={colors.muted} value={entry.quantity} onChangeText={(value) => setEntry((current) => ({ ...current, quantity: value }))} style={[styles.textInput, { color: colors.text, borderColor: colors.border, backgroundColor: colors.surfaceRaised }]} />
          <TextInput placeholder="Amount" placeholderTextColor={colors.muted} keyboardType="numeric" value={entry.amount} onChangeText={(value) => setEntry((current) => ({ ...current, amount: value }))} style={[styles.textInput, { color: colors.text, borderColor: colors.border, backgroundColor: colors.surfaceRaised }]} />
          <TextInput placeholder="Date" placeholderTextColor={colors.muted} value={entry.date} onChangeText={(value) => setEntry((current) => ({ ...current, date: value }))} style={[styles.textInput, { color: colors.text, borderColor: colors.border, backgroundColor: colors.surfaceRaised }]} />
        </View>
          ...
          <View key={draft.id} style={[styles.draftRow, { borderBottomColor: colors.border, backgroundColor: colors.surfaceRaised }]}>
            ...
            <View style={styles.draftCopy}>
              ...
              <Text variant="small" subtle>
                {draft.account} - {draft.date} - {draft.quantity || 'no quantity'} -{' '}
                {draft.amount ? formatCurrency(Number(draft.amount)) : 'amount pending'}
              </Text>
            </View>
            <View style={styles.draftActions}>
              ...
              <Pressable
                onPress={() => setDrafts((current) => current.filter((item) => item.id !== draft.id))}
        ...
        {confirmed.map((draft) => (
          <View key={draft.id} style={[styles.draftRow, { borderBottomColor: colors.border }]}>
            ...
            <View style={styles.draftCopy}>
              ...
              <Text variant="small" subtle>
                Confirmed locally - {draft.account} - {draft.date} -{' '}
                {draft.amount ? formatCurrency(Number(draft.amount)) : 'amount pending'}
              </Text>
            </View>
          </View>
